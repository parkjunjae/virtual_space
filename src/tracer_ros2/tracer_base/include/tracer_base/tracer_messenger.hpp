...
class TracerMessenger {
 public:
  ...
  void PublishStateToROS() {
    ...
    // Timestamp after state retrieval to avoid publishing stale time.
    // Use system time explicitly to avoid any /clock or sim-time influence.
    current_time_ = rclcpp::Clock(RCL_SYSTEM_TIME).now();
